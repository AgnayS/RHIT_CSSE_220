...
public class TwelveProblems {
	...
	public static int firstDifference(String one, String two) { 
		...
		if(one.equals(two)) 
			{
			return -1;        //How the Hell is Test 3 passing??? foo vs bar
			}
	...
	public static char mostCommonCharacter(String input) {
		...
		{
			a[i]=0;   //Assigning Default Values to all elements of a[]
						//I am using a longer Inefficient Way to count and store frequency, so i can recycle code later.
		} 				//Easier way to check frequency for individual character
		
		
		for (i =0; i<input.length();i++)
		{
			a [((int)input.charAt(i))-97]++;
	 ...
	 */
	// Stuck on timesOccur,  I knew for and if logic, but got was using counters, got idea to use booleans from Jintian
        ...
        for (int i = 0; i < longer.length; i++)
        {
            if (longer[i] == shorter[0]) {
                ...
                for (int j = 0; j < shorter.length; j++) 
                {
                    ...
                    if ( i+j >= longer.length|| shorter[j] != longer[i+j]  )
                    {
                        flag = false;
                        break;
                    }
	...
	public static ArrayList<String> doubleDouble(ArrayList<String> input)
	{
		...
		{
			if (input.get(i).equalsIgnoreCase("double"))
			{
				input.add(i,"double");
				i++;
			}
	...
	{
		ArrayList<String> output = new ArrayList<String>();
		for (int i=0,z=input.length();i<z-2;i++)
		{
