...
public class SunComponent extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		s = new Sun(550, 100, Color.BLUE, 50);
		s.drawOn(graphics2);
		g2.drawRect(515, 65, 120, 120);

		// Draw little suns
		double x = SunComponent.LITTLE_SUNS_X_OFFSET;
		for (int i = 0; i < SunComponent.NUM_LITTLE_SUNS; i++, x += SunComponent.LITTLE_SUN_SEPARATION) {
			s = new Sun(x, SunComponent.LITTLE_SUNS_Y, SunComponent.LITTLE_SUN_COLOR, SunComponent.LITTLE_SUN_SIZE);
			s.drawOn(graphics2);
